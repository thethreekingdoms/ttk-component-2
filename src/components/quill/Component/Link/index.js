...
class Link extends Component {
  ...
  onChange = (action, title, target, targetOption) => {
    if (action === 'link') {
      ...
      let linkifiedTarget
      if(this.props.title){
        linkifiedTarget = target;
      }else {
        links = linkify.match(target);
  ...
  render(): Object {
    ...
    const config = {
      ...
      defaultTargetOption: title ? '_self' : '_blank',
      options: ['link', 'unlink'],
      link: { icon: title ? require('../../../../assets/img/link.png'):require('../../../../assets/img/link_out.png'), className: undefined, title: title ? "应用内打开" : "新窗口打开"},
      unlink: { icon:title ? require('../../../../assets/img/link.png'):require('../../../../assets/img/link_out.png'), className: undefined },
      title
    }
