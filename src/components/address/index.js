...
class AddressComponent extends Component {
	...
	render(){
		let {value,showDetail} = this.props
        let selectWidth = 182,
            selectHeight = 29,
            inputWidth = 306,
            city = this.state.city,
		...
		return (
			<div className={`prefixCls`}>
                <Select getPopupContainer={() => document.querySelector(this.state.node)} disabled={disabled} value={this.state.provinces} style={{height: selectHeight, width: selectWidth, 'margin-right':8 }} onChange={::this.provincesChange} >
                    {
                ...
                <Select  getPopupContainer={() => document.querySelector(this.state.node)} disabled={disabled} value={this.state.citys} style={{height: selectHeight, width: selectWidth, 'margin-right':8 }} onChange={::this.citysChange}>
                    {
                        this.citysInit()
                    }
                </Select>
                <Select getPopupContainer={() => document.querySelector(this.state.node)} disabled={disabled} value={this.state.districts}  style={{height: selectHeight, width: selectWidth, 'margin-right':8 }} onChange={::this.districtsChange}>
                    {
                        this.districtsInit()
                    }
                </Select>
